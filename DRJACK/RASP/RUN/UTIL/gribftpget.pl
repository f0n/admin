...
############################################
  ...
  {  
    ...
    ( $hhmm = $time ) =~ s/:..$//;
    `echo "GRIBARCHIVEGET  $rundayprt $hhmm $hhmm ${filename}         0" >> "$RUNDIR/$MODELTYPE/download.log"`;
    #old `echo "GRIBARCHIVEGET  $rundayprt $hhmm $hhmm ${filename}         0" >> "$GRIBDIR/download.log"`;
...
######################  START OF FORMER BASH SCRIPT  #######################
      ...
      elsif ( $LSFTPMETHOD eq "CURL" )
      {
        ...
        {
         `$UTILDIR/curl -v -s -o "${GRIBDIR}/${filename}" "${gribftpsite}/${FTPDIRECTORY}/${filename}" > $TMPFILE1 2>$TMPFILE2`;
          #4test: print "$UTILDIR/curl -v -s -o ${GRIBDIR}/${filename} ${gribftpsite}/${FTPDIRECTORY}/${filename} > $TMPFILE1 2>$TMPFILE2 \n";
        }
        else
        {
          $ID =~ s/ /:/;
          #silent:  `$UTILDIR/curl -s --user $ID $ftpoptions --disable-epsv -o "${GRIBDIR}/${filename}" "ftp://${gribftpsite}/${FTPDIRECTORY}/${filename}" > $TMPFILE1 2>$TMPFILE2`;
          #verbose:
          `$UTILDIR/curl -v -s --user $ID $ftpoptions --disable-epsv -o "${GRIBDIR}/${filename}" "ftp://${gribftpsite}/${FTPDIRECTORY}/${filename}" > $TMPFILE1 2>$TMPFILE2`;
        }
      }
...
# now gets $localgribfilesize, $remotegribfilesize internally
{
  ...
  ### use file for two fields to match fields written by log_grib_download_size
  ( $hhmm = $time ) =~ s/:..$//;
  `echo "$arg  $rundayprt $hhmm $hhmm ${filename} ${localgribfilesize} = ${ftpmins} min  $kbytespersec Kb/s  @ $serverinfo" >> "$RUNDIR/$MODELTYPE/download.log"`;
  #old `echo "$arg  $rundayprt $hhmm $hhmm ${filename} ${localgribfilesize} = ${ftpmins} min  $kbytespersec Kb/s  @ $serverinfo" >> "$GRIBDIR/DATA/download.log"`;
}
