...
use strict;
use Time::Local;
use vars qw($opt_c $opt_d $opt_e $opt_l $opt_f $opt_h
            $opt_i $opt_o  $opt_p
...
getopts('a:b:c:d:e:f:hi:l:o:p:q:r:s:t:T:u:');
...
if ($opt_h){
  print "Usage:  wrfprep.pl [options]
          ...
          -e ENDTIME (YYYYMMDDHH format)
             ...
             then the -l argument is ignored

          -l RUN_LENGTH (in hours)
...
my $dx = ${${wrfsihash{MOAD_DELTA_X}}}[0];
...
if (! $opt_T){ # We compute times to edit wrfsi.nl
 ...
 if (! $opt_e) { # Determine run length if end not set
   if ( $opt_l) {
     $runlength = $opt_l;
   }elsif ( $opt_f) {
     $runlength = $opt_f;
     ...
     print "Use -l hours to change forecast length.\n";
     $runlength = 24;
   }
 }else{  # Ignore -l because end-time is specified
  if($opt_l){ print "-l $opt_l option ignored because -e $opt_e is set\n";}
  elsif($opt_f){ print "-f $opt_f option ignored because -e $opt_e is set. Use -l vs -f.\n";}
 }
  ...
  foreach (@lbctimes) {
    ...
    while ($nnamesread < $numnames){
      ...
      if ($nfound > 1) {
        ...
        print WPLOG "Consider editing the wrfprep.pl -l, -o or -s flags to match the $lbcname\n";
...
if ($opt_q) {
  ...
  if ((-e "$realexe") and ($opt_r)){
    ...
    if ($opt_r =~ /p/i){
      print PBS "/usr/bin/mpirun -np $nprocreal $realexe >> $reallog 2>&1\n";
...
# No PBS, run on this node
  ...
  if ((-e "$realexe")and($opt_r)){
    ...
    if ($opt_r =~ /p/i){
      # Run mpi version
      system("/usr/bin/mpirun -np $nprocreal $realexe > $reallog");
    }else{
      # Run serial version
