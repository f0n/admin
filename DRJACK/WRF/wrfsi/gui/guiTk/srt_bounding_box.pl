...
sub bbox_corners_etc {
    ...
    if ($calc_gridSpacing eq 1) {
       ...
       # The NY Dimension in the NMM MUST be odd
       if ($projection_type eq 'RL' && !($ny_dim % 2)) { $ny_dim++; }
...
sub can_xy_to_ll_proj {
    ...
    if ($ii<=0 || $ii>$nl_var{XDIM}[0]) {$ii="-";}
    if ($jj<=0 || $jj>$nl_var{YDIM}[0]) {$jj="-";}
    my $my_anchor='w';
...
sub ll_proj_to_xy {
    ...
    if ($x ge 1000) {$x=0;}
    if ($y ge 1000) {$y=0;}
    #print "ll_proj_to_xy x: $x, y: $y  $p_lat, $p_lon\n";
...
sub grid_spacing_adjusted {
  ...
  if ($dx_orig eq $grid_spacing_dkm && $grid_spacing_dkm eq $grid_spacing_km ) { return;}
...
sub display_degrees {
  ...
  $nx_degrees=$grid_spacing_dkm * 0.006500;
  $nx_degrees= sprintf ("%.6f",$nx_degrees);
  $ny_degrees=$grid_spacing_dkm * 0.006040;
  $ny_degrees= sprintf ("%.6f",$ny_degrees);
  $display_degrees = ($nx_degrees + $ny_degrees) / 2.;

  # Update grid distance label.
  $gpoint_dist_lab->configure(-text => 
               "Dist between Grid Pts(km): (or $display_degrees deg)");
